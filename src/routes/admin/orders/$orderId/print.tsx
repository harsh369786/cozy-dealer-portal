  ...
  return (
    <div className="print-job-card mx-auto max-w-2xl bg-white p-8 text-black">
            ...
            <tr key={i} className="border-b border-gray-300">
              ...
              <td className="py-2">
                {item.sizeRequested && item.sizeStandard && item.sizeRequested !== item.sizeStandard
                  ? `${item.sizeRequested} (priced as ${item.sizeStandard})`
                  : item.size}{" "}
                × {item.thickness}
              </td>
              <td className="py-2 text-right">{item.quantity}</td>
              <td className="py-2 text-right">
                <span className="line-through text-gray-500">{formatCurrency(item.mrp)}</span>{" "}
      ...
      <OrderNotesPanel
        ...
        items={order.items}
        className="mt-6 border border-gray-400 p-3 text-sm"
      />
