...
Prod URL https://backrest-pwa.shahharsh143-hs.workers.dev. Health + VAPID verified 200 post-deploy. Gupshup env vars all intact on prod (carried by scripts/patch-wrangler-output.mjs).

### Features shipped this session (all committed in 350c4e2, all built GREEN):
1. **Reward-claim workflow (separate from Orders)** — migration 0045 (ADD-only) adds reward_claims.workflow_status (free text, NO check change), distributor_id, rejection_reason, processing_at/dispatched_at/rejected_at/cancelled_at, approved_by/processed_by/dispatched_by/delivered_by/rejected_by. Legacy `status` CHECK untouched, kept loosely in sync. States: pending_approval->approved->processing->dispatched_from_factory->delivered (+rejected/cancelled). shared/reward-claim-status.ts = types + role transition table (distributor: approve/reject[reason MANDATORY]/deliver; admin_staff: process/dispatch; master_admin: all). api/services/reward-claims.ts: advanceRewardClaimStatus (role auth + distributor scope + mandatory reject reason + stage ts/actor + audit + notify), getRewardClaimDetail (history from stage ts + actor joins), listRewardClaimsScoped (dealer=own, distributor/sales_exec=their dealers, full-access=all). Routes GET/POST /api/v1/reward-claims[/:id][/transition]. notifyRewardClaimStatusChange (dealer+distributor+ops admins). RBAC added rewards:approve/process/deliver (api/types.ts Permission + shared/rbac-permissions). NEW claims created at pending_approval w/ distributor_id from dealer. Client: src/services/reward-claims.ts; distributor routes src/routes/distributor/reward-claims/{index,$claimId}.tsx (queue + approve[ConfirmDialog]/reject[RejectOrderDialog mandatory reason]/deliver + history) + nav in more.tsx; admin rewards/claims.tsx reworked (workflow list + process/dispatch) + NEW claims.$claimId.tsx detail; dealer rewards.tsx history tabs Active/Completed w/ status badge + rejection reason. Shared src/components/shared/reward-claim-history.tsx. i18n rewardClaimStatus.* + distributor.rewardClaims.* + admin.rewards.mark*/claimsDescription + notifications.rewardClaimStatus.* (en+hi). Legacy admin PATCH/undo reward-claims routes + rewards-admin.updateRewardClaimStatus/undoRewardClaim kept (back-compat, unwired from UI). 13/13 transition scratch tests pass. TEST: dealer claim -> distributor Reward Claims page approve/reject -> admin claims process->dispatch -> distributor deliver.
2. **Dealer code standardization** FIRST4-PINCODE[-N] — api/services/dealer-code.ts (generateDealerCode); wired into users.ts createDealerStore + signup-review.ts. Migration 0044 adds dealers.legacy_code (snapshots old code) + backfills. Backfill script scripts/backfill-dealer-codes.mjs (run `node scripts/backfill-dealer-codes.mjs --remote` -> writes scripts/dealer-codes.generated.sql [gitignored/untracked], then wrangler d1 execute --remote --file). NOT yet run on remote (existing dealers keep old codes until backfilled; new dealers get new format automatically).
3. **Order customer details** shown in dedicated section across dealer/distributor/admin(=staff) order views — src/components/shared/customer-details-section.tsx; mapOrderDetail +customerEmail; DistributorOrder +customerEmail. Fallback "Customer details not provided.".
4. **Order timeline "Updated By"** now role-aware — mapOrderDetail timeline SQL CASE: dealer->store_name, distributor->distributor name, else personal name.
5. **Global admin search fix** — SearchBar holds local value + optional debounce; AdminFiltersBar debounces 350ms by default; useAsyncData no longer nulls data on dep change (kept prior rows during refetch) so search inputs never unmount/lose focus. Simplified 6 admin pages (products/orders/claims/explore/assignments/visits) to plain search state.
6. **admin_staff Sales Figure ₹0 fix** — scope.ts resolveReportScope no longer force-denies admin_staff (returns {allowed:true}); admin_staff now sees network-wide reports like master_admin.
7. **Push** — savePushSubscription is now a take-over upsert (endpoint reassigned to current user; fixes "registered to another account"); PushSubscriptionConflictError removed. Added cron retryRecentPushNotifications (re-sends recent unread undelivered). Architecture was already server-driven (createNotification->sendPushForNotifications background); symptom was delivery/config, not polling.
8. **Order status rename** "Out for Delivery" -> "Dispatched from Factory" (label/i18n only; status key out_for_delivery unchanged) incl. notification copy.
9. **Rewards Normal vs Target Based** — reward_catalog.kind ('standard'=Normal spends balance / 'milestone'=Target Based cumulative earned) now wired through admin CRUD (was dropped); RewardEditor type selector + Target Points label; "Additional Reward" renamed "Target Based Reward" in i18n. Delete = soft archive (deleted_at).

### PENDING / follow-ups
- **Gupshup WhatsApp delivery STILL BLOCKED** (session 10) — ticket 273826. Sends return submitted+messageId but nothing delivers; WABA on Optimized Marketing/MM Lite; "Partner Client Certification Needed". Awaiting Gupshup. OTP template needs 2 params (code+purpose) — fixed. mattress_order_placed template still PENDING approval in Gupshup.
- **Dealer-code remote backfill NOT run** (see #2). New dealers get new codes; existing keep old until backfilled.
- Legacy reward-claim admin routes/services unwired but present.
- `WhatsApp Image 2026-09-02...jpeg` still untracked (never commit).
